import { MinaNFT, BackendPlugin, sleep } from "minanft";
import {
  Cache,
  verify,
  JsonProof,
...
import { MapContract, ReducerState } from "./mapcontract";
import {
  MapTransition,
  MapUpdate,
  MapUpdateData,
...
export class NameServicePlugin extends BackendPlugin {
  ...
  public async compile(cache: Cache): Promise<void> {
    ...
    if (
      ...
      NameServicePlugin.mapContractVerificationKey === undefined
    )
      NameServicePlugin.mapContractVerificationKey = (
  ...
  public async create(transaction: string): Promise<string | undefined> {
    ...
    if (isAccepted === true) {
      ...
      proof = await MapUpdate.accept(state, update, address);
    } else {
