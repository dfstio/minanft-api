...
*/
  ...
  const zkAppPrivateKey = PrivateKey.fromBase58(
    "EKFaTGoz755C4wTusVRVURQZ283bMDQxXfVMNEpLV5TLaHwg89ZB", //acc.privateKey
  );
  ...
  /*
    ...
    const tx = await Mina.transaction(
        ...
        () => {
            zkApp.mint(Field.fromJSON(NFT_SECRET), Field(5), root);
        },
    );
  ...
  const tx4 = await Mina.transaction(
    ...
    () => {
      ...
      for (k = 0; k < text.length; k++) {
        ...
        zkApp.check(Field(5), w, Encoding.stringToFields(text.substr(k, 1))[0]);
      }
    },
  );
